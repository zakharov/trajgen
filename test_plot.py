...
def test(request_filename, url):
    ...
    dt = 0.001
    tt = [[traj[0].t_sum[0]], [traj[1].t_sum[0]], [traj[2].t_sum[0]]]
    a = [[traj[0].a[0]], [traj[1].a[0]], [traj[2].a[0]]]
    v = [[traj[0].v[0]], [traj[1].v[0]], [traj[2].v[0]]]
    p = [[traj[0].p[0]], [traj[1].p[0]], [traj[2].p[0]]]
    te = max(traj[0].t_sum[-1], traj[1].t_sum[-1], traj[2].t_sum[-1])
    for i in range(0, 3):
        n = 0;
        t = 0;
        while t < te:
...
if __name__ == '__main__':
    test('test2.json', 'http://api.ruckig.com/calculate')